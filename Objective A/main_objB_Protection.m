...
%n_request_integer = 4; % Integer 1 to 5. 1 chooses traffic matrix 1 and so on.
K = 4; % As in K-shortest paths
...
% 'Writedata2excel.m'
data_stored_matrix_E = nan(200,7);
data_stored_matrix_E(1:length(FSU_highest),1) = FSU_highest;
data_stored_matrix_E(1:length(FSU_total),2) = FSU_total;
data_stored_matrix_E(1:length(link_usage),3) = link_usage;
data_stored_matrix_E(1:length(totalcost_scalar),4) = totalcost_scalar;
data_stored_matrix_E(1:length(utilization_entropy),5) = utilization_entropy;
data_stored_matrix_E(1:length(shannon_entropy),6) = shannon_entropy;
data_stored_matrix_E(1:length(Distances_matrix),7) = Distances_matrix;