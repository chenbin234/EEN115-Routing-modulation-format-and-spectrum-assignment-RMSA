...
function [sortedRequest_matrix] = getSortByDifficulty(traffic_data, topology, K)
% If you want to only sort by the shortest path, have K=1
% Otherwise the difficulty is calculated as the mean of path difficulty of
% K nr of paths for each request
    ...
    sortedRequest_matrix = sortrows(sortedRequest_matrix,4);  % sort based on column with difficulty size
